 ...
 *
 * Block (pr, pc) goes to the rank whose Cartesian coords are (pr, pc).
 * We use MPI_Scatter over cart_comm so that the block index matches
 * the cart rank directly (rank = pr*q + pc after reorder=1).
 */
                           ...
                           MPI_Comm cart_comm)
{
  ...
  if (rank == 0)
  {
    ...
    /* Pack so that cart rank (pr*q+pc) owns block (pr,pc) */
    for (int pr = 0; pr < q; pr++)
      for (int pc = 0; pc < q; pc++)
        for (int i = 0; i < blockN; i++)
          for (int j = 0; j < blockN; j++)
          {
            size_t dst = ((size_t)(pr * q + pc) * blockN + i) * blockN + j;
            size_t src = (size_t)(pr * blockN + i) * N + (pc * blockN + j);
 ...
 * Must use the same cart_comm so that rank-to-(pr,pc) mapping is consistent.
 */
    ...
    for (int pr = 0; pr < q; pr++)
      for (int pc = 0; pc < q; pc++)
        for (int i = 0; i < blockN; i++)
          for (int j = 0; j < blockN; j++)
          {
            size_t src = ((size_t)(pr * q + pc) * blockN + i) * blockN + j;
            size_t dst = (size_t)(pr * blockN + i) * N + (pc * blockN + j);
...
int main(int argc, char **argv)
{
  ...
  /* reorder=1 lets MPI optimize rank placement; we rely on             */
  /* MPI_Cart_coords to recover (row,col) for each cart rank.           */
  /* ------------------------------------------------------------------ */
  int dims[2] = {q, q};
  int periods[2] = {1, 1};
  MPI_Comm cart_comm;
  ...
  /* ------------------------------------------------------------------ */
  /* Gather C blocks back to rank 0 — use cart_comm to match scatter    */
  /* ------------------------------------------------------------------ */
